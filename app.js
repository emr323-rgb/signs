...
function computeEntryTime(entry, ctx){
  ...
  switch(entry.type){
    ...
    case "weeklyEarliestZman": {
      ...
      const res = ctx.weekAgg?.earliest?.[key];
      if (!res) return null;
    ...
    case "weeklyLatestZman": {
      const key = entry.baseField;
      const res = ctx.weekAgg?.latest?.[key];
      if (!res) return null;
      const mins = res.minutes + (entry.offsetMin || 0);
      return minutesToDisplay(mins, fmt);
    }
...
async function computeWeekAggregates(cfg, locationId, anchorDate){
  ...
  const daysToInclude = cfg?.rules?.weekdaySunThu ?? [0,1,2,3,4];

  const weekStart = getWeekStart(anchorDate, weekStartsOn);
  const dates = [];

  // Build list for the whole week, then filter to Sun–Thu, to keep logic simple
  for (let i=0; i<7; i++){
  ...
  // Which fields do we need for the weekly rules?
  ...
  // Also scan schedule entries for weekly rules
  const schedules = cfg?.davening || {};
  for (const group of ["shacharis","mincha","maariv"]){
    for (const entry of (schedules[group] || [])){
      if (entry?.type === "weeklyEarliestZman" || entry?.type === "weeklyLatestZman" || entry?.type === "weeklyLatestZmanOrFixed"){
        if (entry.baseField) needed.add(entry.baseField);
      }
    }
  }

  const ymds = dates.map(formatYMD);
  const dayResults = await Promise.all(ymds.map(ymd => getDay(cfg, locationId, ymd)));
  ...
  return { earliest, latest, weekStartYMD: formatYMD(weekStart), daysConsidered: ymds };
}
...
function renderBadgeWeeklyMinchaMaariv(cfg, ctx){
  ...
  const earliest = ctx.weekAgg?.earliest?.[baseField];
  if (!earliest){
...
async function refreshOnce(){
  ...
  const weekAgg = await computeWeekAggregates(cfg, locationId, anchor);
